 ...
 *      (2) Upload the 'example_btp_sender' and 'example_btp_receiver' execution file to
 *          2 different Unex V2X modules.
 *      (3) Run the 'example_btp_sender' on module A and 'example_btp_receiver' on module B,
...
uint8_t tx_payload_pattern[] = "Hello, Unex example_btp_main.c (SHB/BTP) example";
...
int32_t btp_sender(uint16_t dest_port, uint16_t src_port, uint32_t repeat_cnt, uint32_t tx_delay_micros, uint32_t tx_payload_size);
int32_t btp_receiver(uint16_t dest_port, uint16_t src_port, uint32_t repeat_cnt, uint32_t tx_delay_micros, uint32_t tx_payload_size);
...
int32_t main(int argc, char *argv[])
{
    ...
    printf("Hello, This is Unex's single hop broadcast(SHB) BTP example!\n");
    ...
    btp_receiver(dest_port, src_port, repeat_cnt, tx_delay_micros, tx_payload_size);

#else
    dest_port = 3333; /* Send to. */
    src_port = 5555;
    repeat_cnt = 10; /* Send 10 packets. */
    tx_delay_micros = 1000000; /* 1000000 micros = 1 second. */
    ...
    btp_sender(dest_port, src_port, repeat_cnt, tx_delay_micros, tx_payload_size);
...
int32_t btp_sender(uint16_t dest_port, uint16_t src_port, uint32_t repeat_cnt, uint32_t tx_delay_micros, uint32_t tx_payload_size)
{
    ...
    /* Make sure the inputted payload size is valid. */
    if (tx_payload_size > sizeof(tx_payload_buf)) {
        printf("Invalid TX payload size %u\n", tx_payload_size);
        return -1;
    }
